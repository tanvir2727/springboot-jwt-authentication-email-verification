...
import com.example.demo.payload.responses.UserProfile;
...
public class AuthenticationServiceImplementation implements AuthenticationService {
    ...
    public ResponseEntity<RegisterResponse> registerUser(RegisterRequest registerRequest) {
        ...
        try{
            ...
            Optional<User> existingUserOtp = userRepository.findByEmail(registerRequest.getEmail().trim().toLowerCase());
            if(existingUserOtp.isPresent()){
                ...
                log.info("User with email {} successfully created", registerRequest.getEmail());
                return new ResponseEntity<>(RegisterResponse.builder().message("An email with OTP has been sent to your email address. Kindly verify.").build(),HttpStatus.CREATED);
            }
        }
    ...
    public ResponseEntity<?> loginUser(LoginRequest loginRequest) {
        ...
        try{
            ...
            if(!user.isVerified()){
                return new ResponseEntity<>(GeneralAPIResponse.builder().message("User is not verified").build(),HttpStatus.UNAUTHORIZED);
            }
    ...
    public ResponseEntity<?> verifyUserRegistration(RegisterVerifyRequest registerVerifyRequest) {
        ...
        try{
            ...
            if(cachedOtp == null){
                ...
                log.info("User with email {} successfully verified", emailEntered);
                RegisterVerifyResponse jwtToken = jwtService.generateJwtToken(user);
    ...
    public ResponseEntity<?> resendOtp(ForgotPasswordRequest forgotPasswordRequest) {
        ...
        try{
            ...
            log.error("Failed to send OTP email",email,e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(RegisterResponse.builder().message("Failed to send OTP email. Please try again later.").build());
        }
        catch (ResourceNotFoundException ex){
            ...
            log.error("Failed to send OTP email",email,e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(RegisterResponse.builder().message("Failed to resend OTP. Please try again later.").build());
        }
    }
    ...
    public ResponseEntity<?> myProfile(ForgotPasswordRequest forgotPasswordRequest) {
        ...
        try{
            ...
            return new ResponseEntity<>(UserProfile.builder()
                    ...
                    .isOfficiallyEnabled(user.isVerified())
                    .build(),HttpStatus.OK);
        } catch (ResourceNotFoundException ex){
            log.info("user with email {} not found in database", email);
            return new ResponseEntity<>(GeneralAPIResponse.builder().message("user with this email does not exist").build(),HttpStatus.NOT_FOUND);
